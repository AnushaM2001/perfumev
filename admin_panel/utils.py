...
def assign_awb(shipment_id, payload=None):
    ...
    # Step 1: Assign AWB
    response = requests.post(url, json={"shipment_id": shipment_id}, headers=headers)
    awb_result = response.json()
    print("AWB Assignment Response:", awb_result)
    ...
    if awb_code:
        ...
        if tracking_info.get("current_status", "").lower() == "canceled":
            ...
            if payload:
                ...
                print("🔄 Retry Shiprocket Response:", retry_data)
